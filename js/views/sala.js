...
export const salaView = {
    ...
    render(container, turmaId = null, opcoes = {}) {
        if (typeof container === 'string') container = document.getElementById(container);
        if (!container) return;

        this._lastContainer = container;
        this._isEmbutido = !!opcoes?.embutido;
        ...
        const headerHtml = this._isEmbutido ? `
            <!-- TOOLBAR EMBUTIDA DE MAPA DE SALA -->
            <div class="card" style="padding: var(--spacing-4) var(--spacing-6); display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: var(--spacing-4); background-color: var(--color-white); border: 1px solid var(--color-slate-200);">
                <div style="display: flex; align-items: center; gap: 0.5rem;">
                    <span style="font-size: 0.875rem; font-weight: 800; color: var(--color-slate-700);">
                        <i class="fas fa-chair" style="color: #0284c7;"></i> Disposição Física das Carteiras
                    </span>
                    <span class="badge" style="background-color: #f0f9ff; color: #0369a1; font-weight: 800;">
    ...
    ajustarGrid(deltaLinhas, deltaColunas) {
        ...
        const target = this._lastContainer || document.getElementById('turma-aba-content') || document.getElementById('view-container');
        if (target) this.render(target, this.currentTurmaId, { embutido: this._isEmbutido });
    },
