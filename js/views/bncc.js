/**
 * VIEW - Consulta BNCC Pro 2026 (Refatorado Final)
 * Funcionalidade: Motor de busca inteligente, Filtro Estrito de Ano e Extração de Eixos.
 */
const bnccView = {
    selecionarCallback: null,
    dataCache: {},

    // Cores padronizadas por área de conhecimento
    coresComponentes: {
    ...
    // Mapa fixo apenas para popular o dropdown inicial
    mapaEixosBNCC: {
    ...
    async render(containerId, preNivel = null, preSerie = null, callbackExterno = null) {
    ...
    async pesquisar() {
        ...
        try {
            ...
            // Lógica para encontrar a lista correta de habilidades (evita pegar só competências)
            ...
            let filtrados = dados.filter(item => {
                ...
                if (anoBusca && anoBusca !== "Todos") {
                    if (nivel === "Ensino Fundamental") {
                        ...
                        // Pega o par de dígitos do ano (ex: EF[07]MA01)
                        const parAno = codigo.substring(2, 4); 

                        // Tabela da verdade de ciclos e anos
                        ...
                        // Se o código não pertence ao ano/ciclo selecionado, remove.
    ...
    renderCards(data, categoria) {
        ...
        container.innerHTML = data.map(item => {
            ...
            if (this.selecionarCallback) {
                ...
                btnAcao = `<button onclick="controller.copyToClipboard('${textoEscapado}')" class="shrink-0 text-slate-300 hover:text-primary p-2 transition-colors" title="Copiar"><i class="far fa-copy text-lg"></i></button>`;
            }
    ...
    _extrairCodigo(item) {
        // Tenta achar em chaves específicas primeiro
    ...
    _processarItem(item, categoriaDefault) {
        ...
        if (!descricao) {
            ...
            // Pega o maior texto que não é o código
            descricao = valoresSemCodigo.sort((a, b) => b.length - a.length)[0] || "";
        }
        // Limpeza básica da descrição
        ...
        for (let chaveCandidata of chavesEixo) {
            ...
            if (chaveReal && item[chaveReal]) {
                titulo = item[chaveReal];
                break;
            }
        }

        // 2. Busca por Colunas Genéricas (Varredura de Fallback)
        if (!titulo || titulo === "Geral") {
            const colunasCandidatas = ['Column1', 'Column 1', 'Column3', 'Column 3', 'Column4', 'Column 4', 'Column5', 'Column 5'];
            // Adiciona a primeira chave do objeto como candidata se não estiver na lista (ex: "Língua Portuguesa")
            ...
            for (let col of colunasCandidatas) {
                if (item[col]) {
                    ...
                    // Filtros de Rejeição para não pegar lixo
                    if (val.toLowerCase() === categoriaDefault.toLowerCase()) continue; // Igual à Categoria
                    if (val === codigo) continue; // Igual ao código
                    if (val === descricao || val.length > 100) continue; // Igual à descrição ou muito longo
                    if (/^(\d+[º°]|\d+(\s*;)?)+$/.test(val)) continue; // Parece Ano (1º, 2º)
                    if (['COMPONENTE', 'ANO/FAIXA', 'HABILIDADES', 'OBJETOS DE CONHECIMENTO', 'CAMPOS DE ATUAÇÃO'].includes(val)) continue; // Cabeçalhos
...
window.View.renderBncc = (id, n, s, cb) => bnccView.render(id, n, s, cb);