...
export const mensalView = {
                ...
                <div class="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 mb-6 flex flex-col md:flex-row gap-4 items-center justify-between sticky top-0 z-30">
                    ...
                    <div class="relative w-full md:w-72">
                        ...
                        <select id="mensal-selecao-turma" name="mensal-selecao-turma" aria-label="Selecionar Turma"
                            ...
                            ${turmas.length > 0
                                ? turmas.map(t => `<option value="${t.id}" ${t.id == this.currentTurmaId ? 'selected' : ''}>${escapeHTML(t.nome)} - ${t.disciplina || 'Geral'}</option>`).join('')
    ...
    gerarMiniCard(habilidade, turmaId, mes) {
        const codigoSafe =escapeHTML(habilidade.codigo || "").replace(/'/g, "");
        const descSafe = escapeHTML(habilidade.descricao || "").replace(/"/g, '&quot;');
        const eixo = escapeHTML(habilidade.objeto || habilidade.eixo || habilidade.componente || "Habilidade");
        const cor = escapeHTML(habilidade.cor || model.coresComponentes[habilidade.componente] || "#64748b");
        return `
