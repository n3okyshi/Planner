/**
 * VIEW - Planejamento Mensal (Cronograma Anual)
 * Funcionalidade: Grid de 12 meses com seletor de turma.
 */
const mensalView = {
    currentTurmaId: null, // Estado local
            ...
            <div class="fade-in pb-24">
                <div class="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 mb-6 flex flex-col md:flex-row gap-4 items-center justify-between sticky top-0 z-30">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-500">Visão geral mês a mês.</p>
                    </div>
    ...
    gerarGradeMeses(turma) {
        ...
        const colunasHtml = this.meses.map(mes => {
            ...
            const mesAbrev = mes.substring(0, 3);
            ...
            return `
                <div class="flex flex-col bg-slate-50 rounded-xl border border-slate-200 overflow-hidden group/col hover:border-slate-300 transition-colors h-[400px] shadow-sm">
                    ...
                    <div class="p-3 border-b border-slate-200 bg-white flex justify-between items-center shadow-sm z-10 sticky top-0">
                        <span class="text-xs font-black text-slate-600 uppercase tracking-widest pl-1">${mesAbrev}</span>
                        ...
                        <div class="flex items-center gap-1">
                            ...
                            <button onclick="controller.openSeletorBnccMensal(${turma.id}, '${mes}')" 
                    ...
                    <div class="p-2 flex-1 overflow-y-auto custom-scrollbar space-y-2 bg-slate-100/50">
                        ${!isVazio ? habilidades.map(h => this.gerarMiniCard(h, turma.id, mes)).join('') : `
                            <button onclick="controller.openSeletorBnccMensal(${turma.id}, '${mes}')" 
                                    class="w-full h-full flex flex-col items-center justify-center text-slate-300 border-2 border-dashed border-slate-200 rounded-lg hover:border-indigo-400 hover:bg-white hover:text-indigo-500 transition-all opacity-60 hover:opacity-100 group/empty">
                                <i class="fas fa-plus mb-1 group-hover/empty:scale-110 transition-transform"></i>
        ...
        return `
            <div class="animate-slideIn">
                <div class="flex items-center gap-2 mb-4 px-1">
                    <span class="px-2 py-1 bg-slate-200 text-slate-600 text-[10px] font-bold uppercase rounded-md tracking-wider">Cronograma</span>
                    <h3 class="text-sm font-bold text-slate-700">${turma.nome}</h3>
                </div>
    ...
    gerarMiniCard(habilidade, turmaId, mes) {
        ...
        return `
            <div class="bg-white p-2.5 rounded-lg border-l-[3px] shadow-sm relative group hover:shadow-md hover:-translate-y-0.5 transition-all cursor-help border border-slate-200" 
                ...
                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-tighter truncate mb-1">${eixo}</p>
                <p class="text-[10px] text-slate-600 line-clamp-3 leading-snug font-medium border-t border-slate-50 pt-1">${habilidade.descricao}</p>
            </div>
        `;
    },
    ...
    estadoVazio() {
        return `
            <div class="text-center py-20 bg-slate-50 border-2 border-dashed border-slate-200 rounded-3xl mt-10">
                <i class="far fa-calendar-times text-4xl text-slate-300 mb-4"></i>
                <h3 class="text-xl font-bold text-slate-700">Nenhuma turma cadastrada</h3>
...
window.View.renderMensal = (id) => mensalView.render(id);